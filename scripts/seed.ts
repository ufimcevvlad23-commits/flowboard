...
for (const employee of Object.values(seedData.employees)) {
  ...
  await sql`
    INSERT INTO employees (id, name, login, email, password_hash, password_salt, role, status, created_at)
    VALUES (${employee.id}, ${employee.name}, ${employee.login}, ${employee.email ?? null}, ${passwordHash}, ${salt}, ${employee.role}, ${employee.status}, ${employee.createdAt})
  `;
