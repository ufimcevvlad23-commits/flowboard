      ...
      <div className="modal-content team-content">
        {tab === "employees" ? <>
          {canManage ? <div className="employee-form">
            ...
            <label className="field"><span>Уровень доступа</span><select value={role} onChange={(event) => { const nextRole = event.target.value as EmployeeRole; setRole(nextRole); setCanEditDeadlines(nextRole !== "guest"); }}>{(Object.keys(roleLabels) as EmployeeRole[]).map((value) => <option key={value} value={value}>{roleLabels[value]}</option>)}</select><small className="field-help">{roleDescriptions[role]}</small></label>
