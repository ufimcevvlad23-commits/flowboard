          ...
          <div className="access-list">
            {activeEmployees.map((employee) => {
              ...
              return <div className="access-row" key={employee.id}>
                ...
                {canManage && !locked && employee.role !== "admin" && <button className="icon-button small employee-delete access-delete" onClick={() => void removeEmployee(employee)} disabled={saving || deletingId === employee.id} aria-label={`Удалить сотрудника ${employee.name}`} title="Удалить сотрудника"><Trash2 size={14} /></button>}
